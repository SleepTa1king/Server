...
public class SwimPlayerState : PlayerState
{
    ...
    protected override void OnStep(Player player, float deltaTime)
    {
        if (player.onWater)
        {
            ...
            // 水中加速与面向
            player.WaterAcceleration(inputDirection, deltaTime);
            player.WaterFaceDirection(player.lateralVelocity, deltaTime);
            ...
            if (player.position.y < player.water.bounds.max.y)
            {
                ...
                // 施加向上的水浮力
                player.verticalVelocity += Vector3.up * player.stats.current.waterUpwardsForce * deltaTime;
            }
            else
            {
                // 超出水面高度，垂直速度归零
            ...
            {
                player.verticalVelocity += Vector3.down * player.stats.current.swimDiveForce * deltaTime;
            }

            // 没有输入时，减速
            if (inputDirection.sqrMagnitude == 0)
            {
                player.Decelerate(player.stats.current.swimDeceleration, deltaTime);
            }
        }
        else
        {
