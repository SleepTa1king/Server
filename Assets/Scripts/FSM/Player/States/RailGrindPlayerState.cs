...
public class RailGrindPlayerState : PlayerState
{
    ...
    protected override void OnStep(Player player, float deltaTime)
    {
        ...
        if (player.onRails)
        {
            ...
            // 刹车处理
            HandleDeceleration(player, deltaTime);

            // 冲刺处理
            HandleDash(player, deltaTime);

            // 应用坡度影响
            if (player.stats.current.applyGrindingSlopeFactor)
                m_speed -= factor * multiplier * deltaTime;
    ...
    /// 刹车处理逻辑
    /// </summary>
    protected virtual void HandleDeceleration(Player player, float deltaTime)
    {
        if (player.stats.current.canGrindBrake && player.inputs.GetGrindBrake())
        {
            var decelerationDelta = player.stats.current.grindBrakeDeceleration * deltaTime;
    ...
    /// 磨轨冲刺逻辑
    /// </summary>
    protected virtual void HandleDash(Player player, float deltaTime)
    {
