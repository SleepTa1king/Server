...
public class ItemBox : MonoBehaviour, IEntityContact
{
    ...
    public virtual void Collect(Player player)
    {
        ...
        {
            // 还有未收集的物品
            if (this.m_index < collectables.Length)
            {
                // 如果该物品是隐藏的 -> 直接调用 Collect()
                if (collectables[this.m_index].hidden)
                {
                    collectables[this.m_index].Collect(player);
                }
                else
                {
                    // 否则 -> 激活该物体显示在场景中
                    collectables[this.m_index].gameObject.SetActive(true);
                }

                // 递增索引，防止越界
              this.m_index = Mathf.Clamp(m_index + 1, 0, collectables.Length);
