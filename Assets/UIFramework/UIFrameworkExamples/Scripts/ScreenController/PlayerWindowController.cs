...
public class PlayerWindowController : WindowController<PlayerWindowProperties>
{
    ...
    {
        Signals.Get<PlayerDataUpdatedSignal>().AddListener(OnDataUpdates);
    }

    protected override void RemoveListeners()
    {
        Signals.Get<PlayerDataUpdatedSignal>().RemoveListener(OnDataUpdates);
    }

    protected override void OnPropertiesSet()
    {
        OnDataUpdates(Properties.PlayerData);
    }

    private void OnDataUpdates(List<PlayerDataEntry> data)
    {
    ...
    private void VerifyElementCount(int levelCount)
    {
        ...
        if(currentLevels.Count < levelCount)
        {
            ...
            {
                var newLevel = Instantiate(templateLevelEntry, templateLevelEntry.transform.parent);
