...
public class GlidingPlayerState : PlayerState
{
    ...
    protected override void OnStep(Player player, float deltaTime)
    {
        ...
        // 处理滑翔重力，使下落速度受限
        HandleGlidingGravity(player, deltaTime);
        ...
        player.Accelerate(inputDirection,
            ...
            player.stats.current.topSpeed, deltaTime);
    ...
    /// </summary>
    protected virtual void HandleGlidingGravity(Player player, float deltaTime)
    {
        var yVelocity = player.verticalVelocity.y;

        // 按滑翔重力计算速度
        yVelocity -= player.stats.current.glidingGravity * deltaTime;
