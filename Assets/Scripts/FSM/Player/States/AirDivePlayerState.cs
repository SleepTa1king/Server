...
public class AirDivePlayerState : PlayerState
{
    ...
    protected override void OnEnter(Player player)
    {
        ...
        player.lateralVelocity = player.transform.forward * player.stats.current.airDiveForwardForce; // 向前施加俯冲速度
    }
    ...
    protected override void OnStep(Player player, float deltaTime)
    {
        ...
        if (player.isGrounded)
        {
            ...
            // 普通地面时使用普通摩擦力减速
            player.Decelerate(player.stats.current.airDiveFriction, deltaTime);
